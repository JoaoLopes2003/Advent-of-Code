...
// Returns the size of the smallest directory that, if deleted, frees enough space for the update

int deleteSize (Dir *dir, int size)
{
    int sizeDir = dir->size;

    // Goes through every directory and if the size is above the size needed for the update
    // and it is smaller than the sizes previously checked, sets the size that is going to be deleted to that value

    for (int i = 0;i<dir->spDir;i++)
    {
        int sizeAux = deleteSize(dir->proxDir[i], size);

        if (sizeAux>size && sizeAux<sizeDir)
        {
            sizeDir = sizeAux;
        }
    }
...
int main ()
{
    ...
    // Calculates the minimum size to be removed

    int size = 30000000 - (70000000 - dir->size);

    // Findes the size of the smallest directory that, if deleted, frees enough space for the update

    int sizeDelete = deleteSize (aux, size);
